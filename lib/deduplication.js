...
class DeduplicationProcessor {
  ...
  processAllGenres(moviesByGenre, recentMovieIds = []) {
    ...
    // STRATEGY: Allow popular movies in multiple genres
    // Only strictly deduplicate less popular movies
    const POPULARITY_THRESHOLD_FOR_MULTI_GENRE = 100;

    // First pass: Calculate scores for all movie-genre combinations
    const movieGenreScores = new Map();  // movieId -> { genreCode, score, movie }
    const allMovieGenreScores = [];  // All combinations for popular movies
    ...
    for (const genreCode of allGenreCodes) {
      ...
      for (const movie of movies) {
        ...
        // For popular movies (100+ popularity), allow in multiple genres
        if (movie.popularity >= POPULARITY_THRESHOLD_FOR_MULTI_GENRE) {
          allMovieGenreScores.push({
            movieId: movie.id,
            genreCode,
            score,
            movie
          ...
          // For less popular movies, enforce strict deduplication
    ...
    // Second pass: Assign movies to genres
    const moviesByBestGenre = {};
    for (const genreCode of allGenreCodes) {
      moviesByBestGenre[genreCode] = [];
    }

    // Add popular movies (can appear in multiple genres)
    for (const item of allMovieGenreScores) {
      moviesByBestGenre[item.genreCode].push({
        movie: item.movie,
        score: item.score
      });
    }

    // Add less popular movies (deduplicated to one genre)
