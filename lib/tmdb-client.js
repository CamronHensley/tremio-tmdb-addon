...
class TMDBClient {
  ...
  async discoverMovies(genreId, options = {}) {
    ...
    const params = {
      with_genres: genreId,
      sort_by: sortBy,  // Popularity already factors in recency + engagement
      page,
      'vote_average.gte': minRating || 6.5,  // Quality threshold - works for all eras
      include_adult: false,
      include_video: false,
      language: 'en-US',
    ...
    // Don't use vote_count filter - it's biased against older films
    // TMDB's popularity score already handles quality + engagement across all eras
