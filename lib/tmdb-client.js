...
class TMDBClient {
  ...
  async discoverMovies(genreId, options = {}) {
    const {
      page = 1,
      sortBy = 'popularity.desc',  // Popularity works for theatrical AND streaming
      minVotes = 0,
      withCompanies = null,  // Major studios/platforms filter
      certification = null,
      releaseDateGte,
      releaseDateLte,
      region = 'US'
    ...
    // Get today's date for filtering unreleased movies
    const today = new Date();
    const maxReleaseDate = today.toISOString().split('T')[0];

    const params = {
      with_genres: genreId,
      sort_by: sortBy,
      page,
      'vote_count.gte': minVotes || 50,  // Minimal threshold - just filter complete unknowns
      include_adult: false,
      include_video: false,
      language: 'en-US',
      with_original_language: 'en',  // ONLY English-language originals (no Bollywood/Korean)
      'primary_release_date.lte': maxReleaseDate,  // Only released movies (no future releases)
      without_keywords: '210024|9715',  // Exclude: direct-to-video, TV movie quality
      region
    };

    // Filter by major studios (theatrical releases with real budgets)
    if (withCompanies) {
      params.with_companies = withCompanies;
    }

    // Filter by theatrical certifications (excludes TV movies, direct-to-video)
    if (certification) {
  ...
  async discoverByKeywords(genreId, keywords, options = {}) {
    const {
      page = 1,
      sortBy = 'popularity.desc',
      minVotes = 0,
    ...
    const params = {
      ...
      sort_by: sortBy,
      page,
      'vote_count.gte': minVotes || 10,  // Very low threshold for documentaries (smaller audiences)
      include_adult: false,
      include_video: false,
      language: 'en-US',
      with_original_language: 'en',  // ONLY English-language documentaries
      'primary_release_date.lte': maxReleaseDate,
      region: 'US'
    };

    // For documentaries, we want streaming platforms AND traditional docs producers
    if (withCompanies) {
      params.with_companies = withCompanies;
    }
  ...
  async fetchGenreMoviesByKeywords(genreId, keywords, pages = [1, 2, 3], sortBy = 'popularity.desc', additionalParams = {}) {
    const allMovies = [];

    for (const page of pages) {
      try {
        const response = await this.discoverByKeywords(genreId, keywords, {
          page,
          sortBy,
          ...additionalParams
        });
        ...
        console.error(`Failed to fetch page ${page} for keywords ${keywords}:`, error.message);
      }
    }
