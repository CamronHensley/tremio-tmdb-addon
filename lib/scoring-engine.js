...
class ScoringEngine {
   ...
   * TEMPORARILY DISABLED: All quality thresholds removed to build catalog to 100 per genre
   */
  getStrategyParams() {
    const params = {};
    const today = this.today.toISOString().split('T')[0];

    // TEMPORARILY DISABLED: All minRating and minVotes filters removed
    // This allows maximum unique movies for strict deduplication
    // Will restore quality filters once catalog reaches 100 per genre

    switch (this.strategy) {
      case 'RISING_STARS':
        // Recent movies with mid-tier popularity
        params.releaseDateGte = `${this.currentYear - 2}-01-01`;
        // params.minVotes = 100;  // DISABLED
        break;

      case 'CRITICAL_DARLINGS':
        // params.minRating = 7.5;  // DISABLED
        // params.minVotes = 1000;  // DISABLED
        break;

      case 'HIDDEN_GEMS':
        // params.minRating = 7.0;  // DISABLED
        // params.minVotes = 200;  // DISABLED
        // Lower popularity range handled in scoring
        break;

      case 'BLOCKBUSTERS':
        // params.minVotes = 2000;  // DISABLED
        break;
      ...
      case 'FRESH_RELEASES':
        ...
        // params.minVotes = 50;  // DISABLED
        break;

      case 'TIMELESS_CLASSICS':
        params.releaseDateLte = `${this.currentYear - 3}-12-31`;
        // params.minRating = 7.0;  // DISABLED
        // params.minVotes = 5000;  // DISABLED
        break;

      case 'AUDIENCE_FAVORITES':
        // params.minVotes = 3000;  // DISABLED
        break;
    }
   ...
   * TEMPORARILY DISABLED: Returns true for all movies to build catalog to 100 per genre
   */
  passesQualityThreshold(movie, genreCode) {
    // TEMPORARILY DISABLED: Allow all movies through
    // This maximizes unique movie pool for strict deduplication
    // Will restore quality checks once catalog reaches 100 per genre
    return true;
