...
class ScoringEngine {
   ...
   * Date filters only - no quality filters to avoid 404s
   */
  getStrategyParams() {
    const params = {};
    const today = this.today.toISOString().split('T')[0];

    // Only use date filters - quality filtering done in passesQualityThreshold()
    switch (this.strategy) {
      case 'RISING_STARS':
        // Recent movies only
      ...
      // All other strategies: no date filters, get all movies
      default:
        break;
    }
