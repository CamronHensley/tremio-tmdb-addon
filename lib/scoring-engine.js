...
class ScoringEngine {
   ...
   * Use minimal filters to get recognizable movies without 404s
   */
  getStrategyParams() {
    // Use VERY LOW filters at API level to avoid obscure movies
    // while preventing 404 errors from overly restrictive filters
    return {
      minVotes: 100  // Basic filter: at least 100 votes
    };
  }
  ...
  passesQualityThreshold(movie, genreCode) {
    // Optimized thresholds to capture iconic films with high ratings
    // but lower popularity scores (e.g., classics, critically acclaimed films)
    const minThresholds = {
      minVotes: 300,       // Good vote count (ensures legitimacy)
      minRating: 5.5,      // Decent rating
      minPopularity: 15    // Lower threshold captures classics with high ratings
    };

    // High-quality exception: Allow highly-rated films even with lower popularity
    // This captures classics like Schindler's List, The Dark Knight, etc.
    const isHighQuality = movie.vote_average >= 7.5 && movie.vote_count >= 500;

    if (isHighQuality) {
      // Highly-rated films need minimal popularity (captures classics)
      return movie.popularity >= 5;
    }

    // Standard threshold for other movies
    return (
      movie.vote_count >= minThresholds.minVotes &&
      movie.vote_average >= minThresholds.minRating &&
      movie.popularity >= minThresholds.minPopularity
    );
  }
