...
class CacheManager {
   ...
   * Returns the full catalog with movie details from yesterday
   */
  async getPreviousCatalog() {
    const store = this.getStore();

    try {
      const data = await store.get('catalog-previous', { type: 'json' });
      return data;
    } catch (error) {
  ...
  /**
   * Store current catalog as previous for next run
   * Called at end of update to save today's catalog for tomorrow
   */
  async storePreviousCatalog(catalogData) {
    const store = this.getStore();
