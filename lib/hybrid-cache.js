...
class HybridCache {
  ...
  calculateCacheScore(movie) {
    ...
    let ageModifier = 1.0;
    if (age > 50) {
      // 1950s-1970s: Only keep if rating >= 8.0 AND vote_count > 10000
      ageModifier = (movie.vote_average >= 8.0 && movie.vote_count > 10000) ? 0.9 : 0.3;
    } else if (age > 30) {
      // 1980s-1990s: Slight penalty unless highly rated
      ageModifier = movie.vote_average >= 7.5 ? 0.95 : 0.85;
    } else if (age > 20) {
      // 2000s: No penalty
      ageModifier = 1.0;
    } else if (age > 10) {
