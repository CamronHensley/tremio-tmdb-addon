...
class HybridCache {
  ...
  static mergeWithPrevious(freshMoviesByGenre, previousCatalog, freshCount = 30) {
    ...
    for (const genreCode of genreCodes) {
      ...
      // DON'T backfill from previous catalog - keep natural genre sizes
      // This prevents miscategorization where movies are forced into wrong genres
      // to meet a quota. Each genre should only have movies that naturally belong there.
      const fromPrevious = [];

      // OLD CODE (forced backfill):
      // const needed = MOVIES_PER_GENRE - topFresh.length;
      // const fromPrevious = previousMovies.filter(...).slice(0, needed);
