...
class HybridCache {
  ...
  calculateCacheScore(movie) {
    ...
    // Recency preference: penalize very old movies unless they're exceptional
    ...
    let ageModifier = 1.0;
    if (age > 50) {
      // 1950s-1970s: Keep classics (8.0+ with 10k+ votes) OR decent movies with strong following (7.0+ with 5k+ votes)
      if (movie.vote_average >= 8.0 && movie.vote_count > 10000) {
        ageModifier = 0.9;  // True classics
      } else if (movie.vote_average >= 7.0 && movie.vote_count > 5000) {
        ageModifier = 0.7;  // Decent older movies with following (e.g., Godfather Part III)
      } else {
        ageModifier = 0.3;  // Filter out the rest
      }
    } else if (age > 30) {
      // 1980s-1990s: More lenient - keep if rated 7.0+ OR has strong following
      ageModifier = (movie.vote_average >= 7.0 || movie.vote_count > 3000) ? 0.95 : 0.85;
    } else if (age > 20) {
      // 2000s: No penalty
      ageModifier = 1.0;
    } else if (age > 10) {
      ...
      // Recent (2015+): Strong preference
      ageModifier = 1.1;
    }

    return (popularity + rating + votes) * ageModifier;
  }
